...
export function AdminDashboard() {
  ...
  const loadDashboard = async () => {
    ...
    try {
      ...
      // Khớp dữ liệu từ Backend trả về
  ...
  // Thành phần thẻ chỉ số nhỏ
  const StatCard = ({ title, value, icon, color }: any) => (
    <div className="card cardPad" style={{ display: 'flex', alignItems: 'center', gap: 20, borderLeft: `4px solid ${color}` }}>
  ...
  return (
    <div style={{ animation: 'fadeIn 0.4s', display: 'flex', flexDirection: 'column', gap: 24 }}>
      ...
      {/* 1. HÀNG ĐẦU: CÁC CON SỐ TỔNG QUÁT */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: 20 }}>
        <StatCard title="Tổng Doanh thu" value={stats?.totalRevenue} icon="💰" color="#00C853" />
        <StatCard title="Khách du lịch" value={stats?.totalUsers} icon="👥" color="#7B2CBF" />
        <StatCard title="Đối tác (Merchant)" value={stats?.totalMerchants} icon="🏪" color="#9D4EDD" />
        <StatCard title="Địa điểm (POIs)" value={stats?.totalPois} icon="📍" color="#C77DFF" />
      </div>
