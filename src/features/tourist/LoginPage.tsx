  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.success && res.data?.token) {
        ...
        // Save user info to cookies (token, role, email)
        document.cookie = `userToken=${encodeURIComponent(token)}; path=/; max-age=2592000`;
        document.cookie = `userRole=${encodeURIComponent(normalizedRole)}; path=/; max-age=2592000`;
        if (res.data.user?.email) {
          document.cookie = `userEmail=${encodeURIComponent(res.data.user.email)}; path=/; max-age=2592000`;
        }
